...
*
* Parameters:
* 	[key][inputFile][outputFile][number of runs][number of evals]
*
...
*
* To view the trees generated by divideAndOptimize, the treePrint method call in the main method (line 425) must be uncommented. 
* 	The tree information will be displayed in the following format:
*		Branch: [Branch Number]
*			[operation sequences at that node (example: +2 &32)]
*	The tree is designed with each node having two other nodes branching off from it, beginning with a solitary starting node
* 	The branches are then labeled from left to right, starting with the two branches extending from the starting node and beginning
*	 at the left on each down each level of the tree from there. 
*/
